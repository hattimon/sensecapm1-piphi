...
# Version: 3.3 (ARM‑friendly postgres + uninstall option)
...
# - Adds uninstall option to remove ubuntu-piphi and PiPhi files
...
MESSAGES[pl,updating_compose]="Modyfikacja docker-compose.yml (ARM postgres + GPS + volumes)..."
...
MESSAGES[pl,uninstall_header]="Deinstalacja PiPhi z SenseCAP M1..."
MESSAGES[pl,uninstall_done]="Deinstalacja zakończona. Został tylko oryginalny system balenaOS."
...
MESSAGES[en,updating_compose]="Updating docker-compose.yml (ARM postgres + GPS + volumes)..."
...
MESSAGES[en,uninstall_header]="Uninstalling PiPhi from SenseCAP M1..."
MESSAGES[en,uninstall_done]="Uninstall complete. Only original balenaOS services remain."
...
function uninstall_piphi() {
    msg "separator"
    msg "uninstall_header"

    # stop + remove ubuntu-piphi
    balena stop "$CONTAINER_NAME" 2>/dev/null || true
    balena rm "$CONTAINER_NAME" 2>/dev/null || true

    # wyczyść katalog PiPhi
...
function install_piphi() {
    ...
    # podmień postgres na ARM‑friendly image (13‑alpine)
    sed -i 's/image:[[:space:]]*postgres:13\.3/image: postgres:13-alpine/' docker-compose.yml

    # dodaj GPS do software (devices + environment)
    sed -i '/software:/a \    devices:\n      - "'"$GPS_DEVICE"':'"$GPS_DEVICE"'"' docker-compose.yml
    sed -i '/software:/a \    environment:\n      - "GPS_DEVICE='"$GPS_DEVICE"'"' docker-compose.yml

    # popraw grafanę – używaj nazwanego volume
...
if [ "$LANGUAGE" = "pl" ]; then
    ...
    echo "4 - Odinstaluj PiPhi (usuń ubuntu-piphi i /mnt/data/piphi)"
else
    ...
    echo "4 - Uninstall PiPhi (remove ubuntu-piphi and /mnt/data/piphi)"
fi
msg "separator"
